...
void Awp::attack(Positionable& positionable, const Position& actualPosition, const double& angle) {
    ...
    for (int i = 0; i < this->weaponBulletsPerShot; i++) {
        projectiles.emplace_back(std::make_shared<DirectProjectile>(
                positionable, actualPosition, direction, weaponRange, weaponPrecision, weaponSpeed,
...
WeaponType Awp::getType() { return WeaponType::AWP; }
