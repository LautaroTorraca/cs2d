...
struct Snapshot {
  ...
  Snapshot (const GameStatus& status, const uint8_t& currentRound,
    ...
    const std::vector<PlayerInformation>& playersInfo, const std::vector<DropInformation>& drops, const CoordinateInformation& bombPosition) :
    status(status), currentRound(currentRound), countersWinsRounds(countersWinsRounds), terroristsWinsRounds(terroristsWinsRounds),
  playersInfo(playersInfo), dropsInfo(drops), plantedBombPosition(bombPosition) {}
};
