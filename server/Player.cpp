...
Player::Player(const size_t& id, const std::string& name, const Skin& skin,
...
void Player::collision() { this->pushBack(); }
...
void Player::collision(Damager& damager) { this->receive(damager); }

void Player::collision(Damageable& damageable) { damageable.pushBack(); }
...
void Player::setWeapon(const uint8_t& index) { this->inventory.set(index); }
...
void Player::advance(const double& actualTime) { this->inventory.advance(actualTime); }

void Player::reset() {
    Position initialPosition(0, 0, parser.getPlayerInfo(TILE_SIZE_KEY),
                             parser.getPlayerInfo(TILE_SIZE_KEY));
...
void Player::release(const uint8_t& index) { this->inventory.erase(index); }
...
void Player::noMoreAmmo() { this->setWeapon(KNIFE_INDEX); }

void Player::give(const uint16_t& money) { this->wallet.addMoney(money); }
...
PlayerInfoDTO Player::getInfo() {
    ...
    PlayerInfoDTO info(this->id, this->name, this->skin, coordinateInfo, this->angle,
                       this->wallet.getInfo(), this->healthPoints, weaponsInfo, actualWeaponInfo,
                       this->kills);
    return info;
}
