...
WaitingRoomDialog::WaitingRoomDialog(const QString& playerName, const QString& teamStr,
                                     const QString& skinStr, Protocol& protocol, QWidget* parent):
        QDialog(parent),
...
void WaitingRoomDialog::setupUI() {
    ...
    QString selfEntry =
            QString("%1 - Team: %2 - Skin: %3 - [Waiting ⏳]").arg(playerName, teamStr, skinStr);
        ...
        try {
            ...
            if (currentPlayerNames != lastKnownPlayerNames) {
                ...
                QMetaObject::invokeMethod(
                        this,
                        [this, players = lobby.playersChoices]() { updatePlayerList(players); },
                        Qt::QueuedConnection);
            }
                ...
                QMetaObject::invokeMethod(
                        ...
                        [this]() {
                            ...
                            QLabel* startingLabel =
                                    new QLabel("All players ready! Game starting...");
                            startingLabel->setStyleSheet("font-weight: bold; color: #4CAF50; "
            ...
            QMetaObject::invokeMethod(
                    ...
                    [this, error = std::string(e.what())]() {
                        QMessageBox::warning(this, "Connection Error",
                                             QString("Lost connection to server: %1")
            ...
            try {
                ...
                QMetaObject::invokeMethod(
                        ...
                        [this]() {
                            ...
                            QLabel* confirmLabel =
                                    new QLabel("✅ You are ready! Waiting for other players...");
                            confirmLabel->setStyleSheet(
                                    "color: #4CAF50; font-style: italic; padding: 5px;");
                ...
                QMetaObject::invokeMethod(
                        ...
                        [this, error = std::string(e.what())]() {
                            QMessageBox::warning(this, "Error",
                                                 QString("Failed to send ready status: %1")
                                                         .arg(QString::fromStdString(error)));
                        },
                        Qt::QueuedConnection);
            }
