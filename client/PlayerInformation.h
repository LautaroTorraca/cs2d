...
struct PlayerInformation {
    ...
    std::uint8_t kills;
    PlayerInformation(const size_t& id,
        const std::string& name,
        const Skin& skin,
        const CoordinateInformation& position,
        const double& angle,
        const uint16_t& actualMoney,
        const uint8_t& actualHealthPoints,
        std::vector<WeaponInformation>& weaponsInfo,
        const WeaponInformation& actualWeapon,
        const uint8_t& kills) : id(id), name(name), skin(skin), position(position), angle(angle),
        actualMoney(actualMoney), actualHealthPoints(actualHealthPoints), weaponsInfo(std::move(weaponsInfo)),
        actualWeapon(actualWeapon), kills(kills) {}
};
