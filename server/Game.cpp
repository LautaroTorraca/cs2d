...
void Game::addCounter(const size_t &id, const std::string &name) {
  std::shared_ptr<CounterPlayer> counter = std::make_shared<CounterPlayer>(
      id, name, this->gameParser, this->gameMap);
...
void Game::addTerrorist(const size_t &id, const std::string &name) {
  std::shared_ptr<TerroristPlayer> terrorist =
      std::make_shared<TerroristPlayer>(id, name, this->gameParser,
                                        this->gameMap);
...
Game::Game(GameParser &parser, const uint8_t &rounds)
    : gameParser(std::move(parser)), gameMap(gameParser), rounds(rounds) {
  this->teamAdder.emplace(Team::COUNTER_TERRORISTS,
                          [&](const size_t &id, const std::string &name) {
                            this->addCounter(id, name);
                          });
  this->teamAdder.emplace(Team::TERRORISTS,
                          [&](const size_t &id, const std::string &name) {
                            this->addTerrorist(id, name);
                          });
...
void Game::addPlayer(const size_t &id, const std::string &name,
...
void Game::changeAngle(size_t &id, Coordinate &coordinate) {
  if (!this->players.contains(id))
    return; // TODO: Ver si conviente que estas cosas tiren una ecepcion en su
            // lugar
...
std::map<Coordinate, Tile> Game::getMap() { return this->gameParser.getMap(); }
...
std::vector<DropDTO> Game::getDrops() { return this->gameMap.getDrops(); }
