...
void PlayerInventory::erase(const uint8_t& index) { this->weaponSetter.erase(index); }
...
PlayerInventory::PlayerInventory(const GameParser& gameParser, DropPlacer& weaponPlacer,
                                 Position& position, std::shared_ptr<Weapon>& weaponInHands):
        weaponPlacer(weaponPlacer), position(position), weaponInHands(weaponInHands) {
    ...
    this->weaponSetter.emplace(KNIFE_INDEX, std::make_shared<Knife>(knifeMinDamage, knifeMaxDamage,
                                                                    knifeSpeed, knifeRange));

    double glockDamagePerBullet =
            gameParser.getWeaponInfo(WeaponType::GLOCK, DAMAGE_PER_BULLET_KEY);
    ...
    this->weaponSetter.emplace(
            GLOCK_INDEX, std::make_shared<Glock>(glockDamagePerBullet, glockPrecision, glockRange,
                                                 glockCadence, glockSpeed, glockBulletsPerShot));
...
void PlayerInventory::drop(const uint8_t& index) {
    if (this->weaponSetter.contains(index)) {
        std::shared_ptr<Drop> drop =
                std::make_shared<DropBox>(index, this->weaponSetter.at(index), this->position);
...
void PlayerInventory::addBanned(const uint8_t& index) { this->bannedObjects.emplace_back(index); }

void PlayerInventory::setOwner(const std::shared_ptr<Owner>& owner) { this->owner = owner; }
