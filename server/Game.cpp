...
void Game::addCounter(const size_t& id, const std::string& name, const Skin& skin) {
    std::shared_ptr<CounterPlayer> counter =
            std::make_shared<CounterPlayer>(id, name, skin, this->gameParser, this->gameMap);
...
void Game::addTerrorist(const size_t& id, const std::string& name, const Skin& skin) {
    std::shared_ptr<TerroristPlayer> terrorist =
            std::make_shared<TerroristPlayer>(id, name, skin, this->gameParser, this->gameMap);
...
Game::Game(GameParser& parser, const uint8_t& rounds):
        terrorists(*this),
        counters(*this),
        gameParser(std::move(parser)),
        gameMap(gameParser),
        rounds(rounds),
        currentRound(1),
        terroristsWinsRounds(0),
        countersWinsRounds(0),
        bombPlanted(false),
        status(BUY_TIME),
        timeUntilStart(this->gameParser.getGameInfo(BUY_TIME_DURATION_KEY)) {
    this->teamAdder.emplace(Team::COUNTER_TERRORISTS,
                            [&](const size_t& id, const std::string& name, const Skin& skin) {
                                this->addCounter(id, name, skin);
                            });
    this->teamAdder.emplace(Team::TERRORISTS,
                            [&](const size_t& id, const std::string& name, const Skin& skin) {
                                this->addTerrorist(id, name, skin);
                            });
...
void Game::addPlayer(const size_t& id, const std::string& name, const Team& team,
...
void Game::move(const size_t& id, const Coordinate& displacement) {
    // TODO: Despues, implementar un patron state para sacar estos ifs de comprobacion de cada
    // metodo.
    if (this->status != ON_GOING && this->status != BOMB_PLANTED)
        return;
...
void Game::changeAngle(const size_t& id, const Coordinate& coordinate) {
    if (this->status != ON_GOING && this->status != BOMB_PLANTED)
        return;
    if (!this->players.contains(id))
        return;  // TODO: Ver si conviente que estas cosas tiren una excepcion en su lugar
...
std::vector<std::vector<uint8_t>> Game::getMap() const { return this->gameParser.getMap(); }

void Game::buy(const size_t& id, const ProductType& product) {
    if (this->status != BUY_TIME)
        return;
    if (this->players.contains(id)) {
...
void Game::advance(const double& actualTime) {
    if (this->status == BUY_TIME && actualTime >= this->timeUntilStart)
...
void Game::spawnBomb() {
    ...
    double activationDuration =
            this->gameParser.getWeaponInfo(WeaponType::BOMB, BOMB_ACTIVATION_TIME_KEY);
    double deactivationDuration =
            this->gameParser.getWeaponInfo(WeaponType::BOMB, BOMB_DEACTIVATION_TIME_KEY);
    std::shared_ptr<Weapon> bomb =
            std::make_shared<Bomb>(*this, activationDuration, deactivationDuration);
...
std::vector<DropDTO> Game::getDrops() const { return this->gameMap.getDrops(); }
...
GameInfoDTO Game::getInfo() {
    ...
    return GameInfoDTO(this->status, this->currentRound, this->countersWinsRounds,
                       this->terroristsWinsRounds, playersInfo, dropsInfo, plantedBombPosition);
}
