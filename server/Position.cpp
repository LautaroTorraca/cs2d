...
Position::Position(Position&& other) noexcept:
        reference(std::move(other.reference)), width(other.width), height(other.height) {
    if (this != &other) {
...
bool Position::operator<(const Position& other) const {
    return this->reference < other.reference && this->width == other.width &&
           this->height == other.height;  // TODO: Comprobar que esto funcione bien.
}
...
bool Position::intersects(const Position& position) const {
    ...
    return !(xLimit < positionXStart || xStart > positionXLimit || yLimit < positionYStart ||
