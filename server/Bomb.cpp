...
void Bomb::addTo(Inventory& inventory) {
    // TODO: Fijarse si se puede pasar este codigo pseudo-repetido en todas las weapons a la clase
    // padre.
...
void Bomb::advance(const double& actualTime) {
    ...
    if (this->activationStartTime > 0 &&
        (this->actualTime - this->activationStartTime) >= this->activationDuration) {
        this->finalizable.terroristsWins();
    }

    if (this->lastDeactivationTime > 0 &&
        (this->actualTime - this->lastDeactivationTime) >= PERMISSIVE_RELEASE) {
        this->deactivationStartTime = 0;
        this->lastDeactivationTime = 0;
    }

    if (this->deactivationStartTime > 0 &&
        this->actualTime - this->deactivationStartTime >= this->deactivationDuration) {
