...
Ak::Ak(Ak&& other) noexcept:
        PrimaryWeapon(std::move(other)), timePerBurstBullet(other.timePerBurstBullet) {
    if (this != &other) {
...
void Ak::attack(Positionable& positionable, const Position& actualPosition, const double& angle) {
    ...
    for (int i = 0; i < this->weaponBulletsPerShot; i++) {
        this->timesPerBurstBullet.emplace_back(this->actualTime + this->timePerBurstBullet * i +
                                               RETARDATION);
...
WeaponType Ak::getType() { return WeaponType::AK47; }
