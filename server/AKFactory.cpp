...
std::unique_ptr<Product> AKFactory::getProduct(const uint16_t&) {
    double akDamagePerBullet =
            this->weaponsInfo.getWeaponInfo(WeaponType::AK47, DAMAGE_PER_BULLET_KEY);
    ...
    double akBulletsPerShot =
            this->weaponsInfo.getWeaponInfo(WeaponType::AK47, BULLETS_PER_SHOT_KEY);
    double akBurstSpeed = this->weaponsInfo.getWeaponInfo(WeaponType::AK47, BULLETS_PER_SHOT_KEY);
    return std::make_unique<Ak>(this->cost, akDamagePerBullet, akPrecision, akRange, akCadence,
                                akSpeed, akBulletsPerShot, akBurstSpeed);
}
