...
std::unique_ptr<Product> M3Factory::getProduct(const uint16_t&) {
    double m3DamagePerBullet =
            this->weaponsInfo.getWeaponInfo(WeaponType::M3, DAMAGE_PER_BULLET_KEY);
    ...
    return std::make_unique<M3>(this->cost, m3DamagePerBullet, m3Precision, m3Range, m3Cadence,
                                m3Speed, m3BulletsPerShot, m3SweptAngle);
}
