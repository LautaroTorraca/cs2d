...
void GameLobby::select(const size_t& playerId, const std::string& name, const Team& team,
                       const Skin& skin) {
    if (!this->joinedPlayers.contains(playerId))
        return;
    if (this->teams.at(team).size() == this->parser.getMaxPlayersPerTeam())
        return;
